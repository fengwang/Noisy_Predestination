...
batch_size = 16
zeros = [np.zeros((batch_size, 128, 128, 1)), np.zeros((batch_size, 64, 64, 1)), np.zeros((batch_size, 32, 32, 1)), np.zeros((batch_size, 16, 16, 1))]
...
for loop in range( n_loops ):
    ...
    if loop != 0 and loop % check_intervals == 0:
        ...
        model_directory = f'/raid/feng/cache/self2noise/model_noisy_to_clean_model_{training_index}'
        write_model( f'{model_directory}/s2n_{loop}', s2n )
